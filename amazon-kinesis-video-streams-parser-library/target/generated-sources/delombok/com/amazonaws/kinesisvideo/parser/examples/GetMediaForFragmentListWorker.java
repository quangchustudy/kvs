...
public class GetMediaForFragmentListWorker extends KinesisVideoCommon implements Runnable {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(GetMediaForFragmentListWorker.class);
    ...
    public GetMediaForFragmentListWorker(final String streamName, final List<String> fragmentNumbers, final AWSCredentialsProvider awsCredentialsProvider, final String endPoint, final Regions region, final MkvElementVisitor elementVisitor) {
    ...
    public void run() {
        try {
            ...
            final GetMediaForFragmentListResult result = amazonKinesisVideoArchivedMedia.getMediaForFragmentList(new GetMediaForFragmentListRequest().withFragments(fragmentNumbers).withStreamName(streamName));
            log.info("GetMediaForFragmentList called on stream {} response {} requestId {}", streamName, result.getSdkHttpMetadata().getHttpStatusCode(), result.getSdkResponseMetadata().getRequestId());
            final StreamingMkvReader mkvStreamReader = StreamingMkvReader.createDefault(new InputStreamParserByteSource(result.getPayload()));
