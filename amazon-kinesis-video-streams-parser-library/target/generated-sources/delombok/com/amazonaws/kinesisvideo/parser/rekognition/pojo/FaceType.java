...
public enum FaceType {
    TRUSTED(Color.GREEN, "Trusted"), CRIMINAL(Color.RED, "Criminal"), UNKNOWN(Color.YELLOW, "Unknown"), NOT_RECOGNIZED(Color.PINK, "NotRecognized"), ALL(Color.BLACK, "All");
    private final Color color;
    private final String prefix;

    public static FaceType fromString(String value) {
        for (int i = 0; i < FaceType.values().length; i++) {
            if (FaceType.values()[i].getPrefix().toUpperCase().equals(value.toUpperCase())) return FaceType.values()[i];
        }
    ...
    @SuppressWarnings("all")
    private FaceType(final Color color, final String prefix) {
        this.color = color;
        this.prefix = prefix;
    }
}
