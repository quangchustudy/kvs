...
public class H264BoundingBoxFrameRenderer extends H264FrameRenderer {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(H264BoundingBoxFrameRenderer.class);
    ...
    private H264BoundingBoxFrameRenderer(final KinesisVideoBoundingBoxFrameViewer kinesisVideoBoundingBoxFrameViewer, final RekognizedFragmentsIndex rekognizedFragmentsIndex) {
    ...
    public static H264BoundingBoxFrameRenderer create(final KinesisVideoBoundingBoxFrameViewer kinesisVideoBoundingBoxFrameViewer, final RekognizedFragmentsIndex rekognizedFragmentsIndex) {
        return new H264BoundingBoxFrameRenderer(kinesisVideoBoundingBoxFrameViewer, rekognizedFragmentsIndex);
    }

    @Override
    public void process(final Frame frame, final MkvTrackMetadata trackMetadata, final Optional<FragmentMetadata> fragmentMetadata, final Optional<FragmentMetadataVisitor.MkvTagProcessor> tagProcessor) throws FrameProcessException {
    ...
    private Optional<RekognizedOutput> getRekognizedOutput(final Frame frame, final Optional<FragmentMetadata> fragmentMetadata) {
        ...
        if (rekognizedFragmentsIndex != null && fragmentMetadata.isPresent()) {
            ...
            if (rekognizedOutputs != null) {
                ...
                if (log.isDebugEnabled()) {
                    rekognizedOutputs.forEach(p -> log.debug("frameOffsetInSeconds from Rekognition : {}", p.getFrameOffsetInSeconds()));
                }
                ...
                rekognizedOutput = rekognizedOutputs.stream().filter(p -> isOffsetDeltaWithinThreshold(frameOffset, p)).findFirst();
