...
public class ContinuousGetMediaWorker extends KinesisVideoCommon implements Runnable {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ContinuousGetMediaWorker.class);
    ...
    public void run() {
        ...
        while (!shouldStop.get()) {
            ...
            try {
                StartSelector selectorToUse = fragmentNumberToStartAfter.map(fn -> new StartSelector().withStartSelectorType(StartSelectorType.FRAGMENT_NUMBER).withAfterFragmentNumber(fn)).orElse(startSelector);
                getMediaResult = videoMedia.getMedia(new GetMediaRequest().withStreamName(streamName).withStartSelector(selectorToUse));
                log.info("Start processing GetMedia called for stream {} response {} requestId {}", streamName, getMediaResult.getSdkHttpMetadata().getHttpStatusCode(), getMediaResult.getSdkResponseMetadata().getRequestId());
    ...
    private void updateFragmentNumberToStartAfter(FragmentMetadata f) {
        Validate.isTrue(!fragmentNumberToStartAfter.isPresent() || f.getFragmentNumberString().compareTo(fragmentNumberToStartAfter.get()) > 0);
